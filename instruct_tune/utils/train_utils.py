...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None, wandb_run=None, eval_callback=None, accelerator=None):
    """
    ...
    for epoch in range(train_config.num_epochs):
                    ...
                    if accelerator:
                        ...
                        if not train_config.enable_fsdp or rank == 0:
                            if os.path.exists(best_model_path):
                                shutil.rmtree(best_model_path)
                            model.save_pretrained(best_model_path)
                            torch.save(optimizer.state_dict(), os.path.join(best_model_path, "optimizer.pt"))
                            torch.save(lr_scheduler.state_dict(), os.path.join(best_model_path, "scheduler.pt"))
                    ###
                    if train_config.enable_fsdp:
                        if rank==0:
        ...
        if eval_callback:
            eval_callback(epoch, model, train_config, optimizer, rank)
